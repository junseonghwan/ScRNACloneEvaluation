...
rule correctDepth:
	input:
		expand("/proj/sc_ml/cellline/bulk/OV2295/cna/results/ichorCNA/{tumor}/{tumor}.cna.seg", tumor=config["pairings"]),
		expand("/proj/sc_ml/cellline/bulk/OV2295/cna/results/ichorCNA/{tumor}/{tumor}.seg.txt", tumor=config["pairings"]),
		expand("/proj/sc_ml/cellline/bulk/OV2295/cna/results/ichorCNA/{tumor}/{tumor}.params.txt", tumor=config["pairings"]),
		expand("/proj/sc_ml/cellline/bulk/OV2295/cna/results/ichorCNA/{tumor}/{tumor}.correctedDepth.txt", tumor=config["pairings"]),
		expand("/proj/sc_ml/cellline/bulk/OV2295/cna/results/readDepth/{samples}.bin{binSize}.wig", samples=config["samples"], binSize=str(config["binSize"]))

rule read_counter:
	input:
		lambda wildcards: config["samples"][wildcards.samples]
	output:
		"/proj/sc_ml/cellline/bulk/OV2295/cna/results/readDepth/{samples}.bin{binSize}.wig"
	params:
	...
	resources:
		mem=16
	log:
		"/proj/sc_ml/cellline/bulk/OV2295/cna/logs/readDepth/{samples}.bin{binSize}.log"
	shell:
		"{params.readCounter} {input} -c {params.chrs} -w {params.binSize} -q {params.qual} > {output} 2> {log}"

rule ichorCNA:
	input:
		tum="/proj/sc_ml/cellline/bulk/OV2295/cna/results/readDepth/{tumor}.bin" + str(config["binSize"]) + ".wig",
		norm=lambda wildcards: "/proj/sc_ml/cellline/bulk/OV2295/cna/results/readDepth/" + config["pairings"][wildcards.tumor] + ".bin" + str(config["binSize"]) + ".wig"
	output:
		corrDepth="/proj/sc_ml/cellline/bulk/OV2295/cna/results/ichorCNA/{tumor}/{tumor}.correctedDepth.txt",
		param="/proj/sc_ml/cellline/bulk/OV2295/cna/results/ichorCNA/{tumor}/{tumor}.params.txt",
		cna="/proj/sc_ml/cellline/bulk/OV2295/cna/results/ichorCNA/{tumor}/{tumor}.cna.seg",
		segTxt="/proj/sc_ml/cellline/bulk/OV2295/cna/results/ichorCNA/{tumor}/{tumor}.seg.txt",
		#seg="results/ichorCNA/{tumor}/{tumor}.seg",
		#rdata="results/ichorCNA/{tumor}/{tumor}.RData",
	params:
		outDir="/proj/sc_ml/cellline/bulk/OV2295/cna/results/ichorCNA/{tumor}/",
	...
	resources:
		mem=16
	log:
		"/proj/sc_ml/cellline/bulk/OV2295/cna/logs/ichorCNA/{tumor}.log"
	shell:
